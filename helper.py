...
def distance_between(pos1, pos2, normalize=False, justpos=None):
    #max_dist = np.linalg.norm(np.asarray((game.w-1,game.h-1)) - np.asarray((0,0)))
    #dist = np.linalg.norm(np.asarray(pos1) - np.asarray(pos2))
    #return dist if asblocks else (dist / max_dist)
    dist = None
    max_dist = None
    sign_val = 0
    if justpos==0:
